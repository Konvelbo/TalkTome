...
export const denoiseAudio = createServerFn({ method: 'POST' })
  .validator((data: unknown) => data as { audioBase64: string; mimeType: string })
  .handler(async ({ data }) => {
    ...
    const file = new File([blob], `audio${extFromMime(mimeType)}`, { type: mimeType })
    ...
    if (!response.ok) {
      ...
      throw new Error(`Denoising service error (${response.status}): ${detail.slice(0, 200)}`)
    }
    ...
    return {
      ...
      // Maintenant en accord parfait avec l'API qui renvoie un vrai MP3
      mimeType: 'audio/mpeg',
    }
  })
